...
reg regs[REG_NUM];
//规定 0-9是t系列寄存器(和mips标准不同)，10-17为s系列寄存器，18fp帧指针，19sp栈指针，20ra返回地址，21v0系统调用和返回值，22-25为a系列寄存器(仅用于函数参数)
//mips还有寄存器v1(表达式求值和函数结果)。 zero(常数0)，at(汇编器保留)，k0,k1(中断处理)，此处不用
...
int GetEmptyReg(FILE*file);//在t系列和s系列寄存器中找空余寄存器，没有则选LRU_count值最大的溢出，溢出时将写入内存的代码写入fp
void PrintReg(int index,FILE*file);//打印寄存器名
void printAllCode(char*fname);//向fname中打印所有目标代码
void printObjCode(InterCode c,FILE* file);//将一份中间代码转成目标代码写到fp中
